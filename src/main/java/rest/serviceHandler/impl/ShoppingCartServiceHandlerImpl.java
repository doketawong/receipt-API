...
public class ShoppingCartServiceHandlerImpl implements ShoppingCartServiceHandler {
    ...
    public ResponseEntity<ShoppingResponse> getReceipt(ShoppingRequest shoppingRequest) {
        ...
        Set<ConstraintViolation<ShoppingRequest>> violations = validator.validate(shoppingRequest);
        for (ConstraintViolation<ShoppingRequest> violation : violations) {
            shoppingResponse.setError(violation.getMessage());
            return ResponseEntity.status(ShoppingCartConstant.ERROE_STATUS).body(shoppingResponse);
        }
